...
export default function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (getUserByEmail(trimmed)) { setError('User already exists'); return; }
      const user = createUser(trimmed, password);
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none',
        background: 'radial-gradient(ellipse 70% 50% at 50% 110%, rgba(124,58,237,0.12) 0%, transparent 70%)',
      }} />
      ...
      <div style={{ width: '100%', maxWidth: 400, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.125rem' }}>
            ...
            <div>
              <label htmlFor="signup-password" className="ht-label">Password</label>
              <div style={{ position: 'relative' }}>
                <Lock size={16} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', pointerEvents: 'none' }} />
                <input
                  id="signup-password" data-testid="auth-signup-password"
                  type={showPw ? 'text' : 'password'} className="ht-input" required autoComplete="new-password"
            ...
            <button
              data-testid="auth-signup-submit" type="submit" disabled={loading}
              className="btn btn-primary"
              style={{ width: '100%', padding: '0.875rem', marginTop: '0.25rem', opacity: loading ? 0.7 : 1 }}
            >
              {loading ? 'Creating account…' : (
                <><span>Create account</span><ArrowRight size={17} strokeWidth={2.5} /></>
              )}
            </button>
          </form>
        </div>
